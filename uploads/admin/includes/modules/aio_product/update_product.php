 ...
 */
  ...
  if (isset($_POST['edit_x']) || isset($_POST['edit_y'])) {
    ...
    if (isset($_SESSION['provider_id'])) $tmp_value = zen_db_prepare_input($_SESSION['provider_id']);
    ...
    $sql_data_array = array('products_quantity' => $products_quantity,
                            ...
                            'products_date_added' => zen_db_prepare_input($_POST['products_date_added']),
...
if ($_POST['image_delete'] == 1) {
    ...
    if ($action == 'insert_product') {
      $insert_sql_data = array( 'master_categories_id' => (int)$current_category_id);

	  if (defined('PRODUCTS_ON_QUEUE') && PRODUCTS_ON_QUEUE == 'True') {
	...
	if ($reload == true)
	{
		$type_handler = $zc_products->get_admin_handler(zen_db_prepare_input($_GET['product_type']));
		zen_redirect(zen_href_link($type_handler , 'cPath=' . $cPath . (isset($_GET['page']) ? '&page=' . $_GET['page'] : '') . '&product_type=' . zen_db_prepare_input($_GET['product_type']) . '&pID=' . $products_id . '&action=new_product' . (isset($_GET['search']) ? '&search=' . $_GET['search'] : '')));
	}
