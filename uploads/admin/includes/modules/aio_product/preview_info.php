...
?>
	...
	<table border="0" width="100%" cellspacing="0" cellpadding="0">
         <tr>
           <td align="right">
			<?php
				echo '<input type="submit" name="edit_x" value="' . IMAGE_BACK . '" />&nbsp;&nbsp;';
				  if (isset($_GET['pID'])) {
					echo '<input type="submit" value="' . IMAGE_UPDATE . '" />';
				  echo '&nbsp;&nbsp;<input type="submit" value="' . UPDATE_N_RELOAD . '" name="reload" />';
				  } else {
					echo '<input type="submit" value="' . IMAGE_INSERT . '" />';
				  echo '&nbsp;&nbsp;<input type="submit" value="' . INSERT_N_RELOAD . '" name="reload" />';
				  }
				 echo '&nbsp;&nbsp;<a href="' . zen_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . (isset($_GET['pID']) ? '&pID=' . $_GET['pID'] : '') . (isset($_GET['page']) ? '&page=' . $_GET['page'] : '') . (isset($_GET['search']) ? '&search=' . $_GET['search'] : '')) . '" class="button">' . IMAGE_CANCEL . '</a>';
			?>
		   </td>
		 </tr>
    </table>
